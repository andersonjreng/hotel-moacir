...
export class ApiService {
  ...
  getQuartos(): Observable<Quartos[]> {
    const urlQuartos = `${this.endpoints.baseQuartos()}`
    return this.httpClient.get<Quartos[]>(urlQuartos);
  }

  deleteQuartoPorId(id: number): Observable<Quartos[]> {
    const urlQuartos = `${this.endpoints.baseQuartoPorId(id)}`
    return this.httpClient.delete<Quartos[]>(urlQuartos);
  }

  postQuartos(dados: Quartos): Observable<Quartos[]> {
    const urlQuartos = `${this.endpoints.baseQuartos()}`
    return this.httpClient.post<Quartos[]>(urlQuartos, dados);
  }

  putQuartos(dados: any, id: number): Observable<any> {
    const urlCompany = `${this.endpoints.baseQuartoPorId(id)}`
    return this.httpClient.put<any>(urlCompany, dados);
  }

  getHospedes(): Observable<Hospede[]> {
    const urlHospedes = `${this.endpoints.baseHospedes()}`
    return this.httpClient.get<Hospede[]>(urlHospedes);
  }

  getReservas(): Observable<Reserva[]> {
    const urlReservas = `${this.endpoints.baseAgendamentos()}`
    return this.httpClient.get<Reserva[]>(urlReservas);
  }

  postReserva(dados: Reserva): Observable<Reserva[]> {
    const urlReserva = `${this.endpoints.baseAgendamentos()}`
    return this.httpClient.post<Reserva[]>(urlReserva, dados);
  }
